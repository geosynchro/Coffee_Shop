...
function ViewItem({itemView}){
    const [reviews, setReviews] = useState(false)
    console.log(itemView)

    // const reviewsToDisplay = itemView.reviews.map(review => <ItemReview key={review.id} id={review.id} review={review.review_text}/>)
      ...
      <div className="border-t border-gray-200">
        <dl>
          ...
          <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2"> 4/5  <button onClick={displayReviews} className="bg-gray-200 rounded-md px-1 py-1 ml-4 shadow-md shadow-gray-500 border-1 border-black">View Reviews</button></dd>
          </div>

          <div>
              {reviews ? <h1>yo</h1> : null}
          </div>
